  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        _push(`<div class="catalog-layout"><aside class="category-panel rounded-4 p-3"><div class="small text-uppercase fw-bold text-muted mb-3 category-title"><i class="bi bi-funnel me-2 text-primary"></i>Filter kategori</div><div class="category-list"><button class="${ssrRenderClass([{ active: !unref(selectedCat) }, "category-option"])}"><span><i class="bi bi-grid me-2"></i>Semua materi</span><small>${ssrInterpolate(unref(prompts)?.length || 0)}</small></button><!--[-->`);
        ...
        ssrRenderList(unref(paginatedPrompts), (item, index) => {
          _push(`<div class="col-12 col-sm-6 col-xl-4 col-xxl-3 reveal-card" style="${ssrRenderStyle({ "--delay": `${index * 40}ms` })}"><article class="learning-card h-100 rounded-4 overflow-hidden"><div class="thumb-wrap position-relative"><img${ssrRenderAttr("src", `https://img.youtube.com/vi/${unref(getYouTubeID)(item.youtube_url)}/mqdefault.jpg`)} class="w-100 h-100 object-fit-cover" alt="Thumbnail tutorial"><span class="play-badge"><i class="bi bi-play-fill"></i></span><span class="category-badge">${ssrInterpolate(item.categories?.name || "AI")}</span></div><div class="card-body p-4 d-flex flex-column"><div class="small text-primary fw-semibold mb-2"><i class="bi bi-play-circle me-1"></i>Tutorial &amp; Prompt</div><h5 class="fw-bold line-clamp-2 mb-3">${ssrInterpolate(item.title)}</h5><div class="mt-auto d-grid gap-2">`);
...
//# sourceMappingURL=index-CFNpp94D.js.map
